...
const LEAD_ENDPOINT = 'https://flyer-club.com/f/sendlead';
...
export async function formSubmit(json) {
  ...
  let response = await axios(LEAD_ENDPOINT, {
    method: 'POST',
...
export async function formSubmitTicket() {
  ...
  const sendTicket = await axios(LEAD_ENDPOINT, {
    method: 'POST',
    body: ticket,
  });
  if (sendTicket.status === 200) {
    dataLayer.push({ event: 'form_sent' });
  } else {
